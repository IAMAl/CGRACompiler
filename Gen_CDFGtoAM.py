...
def AMComposer( ZERO_REMOVE, mode, r_file_path, r_file_name, w_file_path, w_file_name ):
    """
    Adjacency Matrix and Node-List Composer
    ...
    #   Parsing Data-Flow Graph with Node-ID
    openfile = r_file_path + r_file_name+"_dfg"+".dot"
    ...
    with open(openfile, "r") as dot_file:
        for present_line in dot_file:
            ...
            tmp_line = []
            if len(present_line) > 1:
                if not "getelementptr"in present_line[0]:
                    present_line[1] = present_line[1].split('\"[')[0]
                if len(present_line[1]) > 1:
                    tmp = present_line[1][1].split(' label=')
                    if len(tmp) > 1:
                        tmp_line.append(present_line[1][0])
                        ...
                        tmp_line.append(present_line[1])
                        tmp_line.append(present_line[0])
                        present_line = tmp_line
                else:
                    tmp_line.append(present_line[0])
        ...
        if find_dst:
            if len(nodes) > 1:
                ...
                if len(node_list[index]) > 2:
                    node_list[index] = node_list[index][0:3]+[" "+src_index]+[node_list[index][3]]
    ...
    for index, leaf_node in enumerate(leaf_node_list):
        ...
        if not find and leaf_node[1] != leaf_node[2]:
    ...
    with open(openfile, "w") as node_list_file:
        for index, node in enumerate(node_list):
            ...
            # Write src Edge
            if len(node) == 5 and mode == "dst_append":
                # Two Sources
                node[2] = node[2].replace('\"','')
                node_list_file.write(" "+node[2]+" "+node[3].replace(" ", '')+"\n")
            elif len(node) == 4 and mode == "dst_append":
                # One Source
                node[2] = node[2].replace('\"','')
    ...
    openfile = w_file_path + w_file_name+"_nemonic_list_inv.txt"
    with open(openfile, "w") as node_list_file:
        for index, node in enumerate(node_list):
            # Write dst Node
            if len(node) == 5:
                # Two Sources
                node_list_file.write(str(node[0])+" "+node[4].replace("\n", '')+"\n")
            elif len(node) == 4:
                # One Source
    ...
    openfile = w_file_path + w_file_name+"_nemonic_list.txt"
    with open(openfile, "w") as node_list_file:
        for index, node in enumerate(node_list):
            # Write dst Node
            if len(node) == 5:
                # Two Sources
                node_list_file.write(str(len(node_list) - node[0])+" "+node[4].replace("\n", '')+"\n")
            elif len(node) == 4:
                # One Source
...
def Gen_CDFGtoAM(r_file_path, r_file_name, w_file_path, ZERO_REMOVE, DST_APPEND):
